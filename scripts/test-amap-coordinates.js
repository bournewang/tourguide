...
/**
 * Test script for 高德地图 (Amap) Coordinate Service
 * This script tests the 高德地图 API key and coordinate fetching functionality
 */
async function testAmapCoordinates() {
  console.log('🧪 Testing 高德地图 (Amap) Coordinate Service');
  console.log('==========================================');
  
  // Initialize the service
  const coordinateService = new AmapCoordinateService();
  ...
  const testCityFile = path.join(tempDir, 'test-city-amap.json');
  const testCityData = {
    city: '测试城市',
    province: '测试省份',
...
// Run the test
testAmapCoordinates().catch(error => {
